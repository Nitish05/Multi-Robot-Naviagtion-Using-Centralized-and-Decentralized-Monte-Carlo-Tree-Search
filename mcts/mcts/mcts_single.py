...
class Robot:
    def __init__(self, robot_name, params, node):
        ...
        if robot_config is None:
            node.get_logger().error(f"Robot {robot_name} not found in configuration")
        ...
        node.get_logger().info(f'MCTS initialized for {self.namespace}')
        ...
        self.odom_sub = node.create_subscription(Odometry, f'{self.namespace}/odom', self.odom_callback, qos_profile)
        self.scan_sub = node.create_subscription(LaserScan, f'{self.namespace}/scan', self.scan_callback, qos_profile)
        self.cmd_pub = node.create_publisher(Twist, f'{self.namespace}/cmd_vel', qos_profile)
    ...
    def scan_callback(self, msg):
        if self.position is None or self.heading is None:
            self.node.get_logger().debug(f'{self.robot_name}: Waiting for Odom...')
            return
        ...
        if len(ranges) == 0:
            ...
            self.closest_obstacle_angle = None
            self.obstacles_detected = False
            self.node.get_logger().debug(f'{self.robot_name}: No obstacles detected.')
            return
        ...
        front_angles = (angles > -math.pi/4) & (angles < math.pi/4)  # 90-degree FOV
        ...
        if len(ranges) > 0:
            ...
            self.node.get_logger().debug(f"{self.robot_name}: Detected {len(self.obstacles)} obstacles.")
        else:
            self.closest_obstacle_angle = None
    ...
    def plan_action(self):
        ...
        if self.position is None or self.heading is None:
            self.node.get_logger().debug(f'{self.robot_name}: Waiting for information from sensors...')
            return

        if self.goal is None:
            self.node.get_logger().debug(f'{self.robot_name}: Waiting for a goal point to be set...')
            return

        dist_to_goal = np.linalg.norm(np.array(self.position) - self.goal)
        self.node.get_logger().info(f"{self.robot_name}: Distance to goal: {dist_to_goal:.2f} meters")

        # If close enough to the goal, stop and print final metrics
        if dist_to_goal < self.goal_threshold:
            self.stop_robot()
            self.node.get_logger().info(f'{self.robot_name}: Goal Reached')
        ...
        for _ in range(self.mcts_iterations):
            ...
            if reward == -1000:
                ...
                self.node.get_logger().warn(f'{self.robot_name}: Collision detected during planning!')
                break  # Stop planning upon collision detection

        if collision_detected:
            self.stop_robot()
            self.node.get_logger().info(f'{self.robot_name}: Stopping due to collision')
        ...
        self.node.get_logger().info(
            f"{self.robot_name}: Iteration: {self.iteration_count}, Time Elapsed: {elapsed_time:.2f}s, "
    ...
    def stop_robot(self):
        ...
        self.node.get_logger().info(f'{self.robot_name}: Stopping Robot')

    def print_final_metrics(self, total_time):
        self.node.get_logger().info("***** Final Metrics *****")
        self.node.get_logger().info(f"{self.robot_name}: Total time elapsed: {total_time:.2f}s")
        self.node.get_logger().info(f"{self.robot_name}: Total iterations (action planning steps): {self.iteration_count}")
        self.node.get_logger().info(f"{self.robot_name}: Total MCTS calls: {self.mcts_calls}")
        self.node.get_logger().info(f"{self.robot_name}: Total expansions: {self.total_expansions}")
        self.node.get_logger().info(f"{self.robot_name}: Total rollouts: {self.total_rollouts}")
        self.node.get_logger().info(f"{self.robot_name}: Collision Rate: {self.collision_count / self.iteration_count if self.iteration_count > 0 else 0:.2f}")
        self.node.get_logger().info(f"{self.robot_name}: Robot successfully reached its goal.")
...
class MCTSManager(Node):
    def __init__(self):
        super().__init__('decentralized_mcts_manager')
        self.params = GlobalParameters()

        # Initialize robots
        self.robots = {}
        for robot_name in self.params.robots.keys():
            try:
                robot = Robot(robot_name, self.params, self)
                self.robots[robot_name] = robot
            except ValueError as e:
                self.get_logger().error(str(e))

        if not self.robots:
            self.get_logger().error("No robots to manage. Shutting down.")
            rclpy.shutdown()
            return

        # Create a timer to plan actions for all robots
        self.plan_timer = self.create_timer(0.5, self.plan_actions)
